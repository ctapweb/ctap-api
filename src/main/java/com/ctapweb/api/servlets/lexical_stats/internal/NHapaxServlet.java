...
@WebServlet(description = "Service for counting the number of hapax legomena (words that appear only once)",
urlPatterns = { NHapaxServlet.URL_PATTERN })
public class NHapaxServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	public static final String URL_PATTERN = "/lexical_stats/internal/n_hapax/*";
	public static final String STAT_NAME = "n_hapax";
	...
	private Item runAnalysisAndGetResult(TextTemplate text) {
		...
		double ttr = LexicalCounts.countNHapax(annotation);

		//create a result item
		logger.trace("Analyzed text \"{}\", with result {}.", 
				textContent.length() > 50 ? textContent.substring(0, 50) + "..." : textContent, ttr);
		resultItem = Item.builder()
				...
				.addProperty(Property.value(ResultItem.KEY_RESULT_VALUE, ValueFactory.createValue(ttr)))
				.build();
